  ...
  private mapMarketplaceOrderFromDB(order: { id: string; metadata?: Record<string, unknown>; items: Array<{ id: string; productId: string; quantity: number; price: number }> }): MarketplaceOrder {
    return {
      ...
      items: order.items.map((item: { id: string; productId: string; quantity: number; price: number }) => ({
        ...
        price: item.price,
        commission: (item.metadata as Record<string, unknown> & { commission?: number })?.commission || 0,
        vendorPayout: (item.metadata as Record<string, unknown> & { vendorPayout?: number })?.vendorPayout || 0,
      })),
      subtotal: order.totalAmount,
      commission: (order.metadata as Record<string, unknown> & { commissionAmount?: number })?.commissionAmount || 0,
