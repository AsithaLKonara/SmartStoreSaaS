...
export class SriLankaCourierService extends EventEmitter {
  ...
  async trackShipment(trackingNumber: string, courierCode: string): Promise<TrackingInfo> {
    ...
    try {
      ...
      await this.syncTrackingEvent(trackingInfo, (courier as { organizationId?: string }).organizationId || '');
  ...
  private async trackAramex(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      events: data.Shipments[0]?.ShipmentEvents?.map((event: Record<string, unknown>) => ({
        status: event.Status,
  ...
  private async trackDHL(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      estimatedDelivery: data.shipments[0]?.estimatedDeliveryDate ? 
        new Date(data.shipments[0].estimatedDeliveryDate) : undefined,
      events: data.shipments[0]?.events?.map((event: Record<string, unknown>) => ({
        status: event.status,
        location: event.location,
        timestamp: new Date(event.timestamp),
        description: event.description
  ...
  private async trackFedEx(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      events: data.output?.completeTrackResults[0]?.trackResults[0]?.scanEvents?.map((event: Record<string, unknown>) => ({
  ...
  private async trackUPS(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      events: data.trackResponse?.shipment[0]?.package[0]?.activity?.map((event: Record<string, unknown>) => ({
  ...
  private async trackCeylinco(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      events: data.trackingEvents?.map((event: Record<string, unknown>) => ({
        status: event.status,
        location: event.location,
        timestamp: new Date(event.timestamp),
        description: event.description
  ...
  private async trackSkynet(trackingNumber: string, courier: CourierConfig): Promise<TrackingInfo> {
    ...
    return {
      ...
      description: data.statusDescription || '',
      estimatedDelivery: data.estimatedDelivery ? new Date(data.estimatedDelivery) : undefined,
      events: data.trackingHistory?.map((event: Record<string, unknown>) => ({
        status: event.status,
        location: event.location,
        timestamp: new Date(event.timestamp),
        description: event.description
    ...
    try {
      ...
      await prisma.shipment.create({
        data: {
          ...
          status: shipment.status as 'PENDING' | 'ASSIGNED' | 'PICKED_UP' | 'IN_TRANSIT' | 'OUT_FOR_DELIVERY' | 'DELIVERED' | 'FAILED' | 'RETURNED', // Cast to ShipmentStatus enum values
