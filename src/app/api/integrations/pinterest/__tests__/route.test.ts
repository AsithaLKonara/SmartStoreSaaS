...
jest.mock('@/lib/integrations/pinterest/pinterestService', () => ({
  PinterestService: jest.fn().mockImplementation(() => ({
    testConnection: jest.fn(),
    getOrCreateCatalog: jest.fn(),
  })),
}));

describe('/api/integrations/pinterest', () => {
  const mockSession = {
    user: {
      id: 'user-1',
  ...
  describe('GET', () => {
    it('should return 401 for unauthorized requests', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/integrations/pinterest');
    ...
    it('should return integration when found', async () => {
      ...
      (prisma.pinterestIntegration.findFirst as jest.Mock).mockResolvedValue(mockIntegration);
    ...
    it('should return 404 when integration not found', async () => {
      (prisma.pinterestIntegration.findFirst as jest.Mock).mockResolvedValue(null);
    ...
    it('should return 500 on server error', async () => {
      (prisma.pinterestIntegration.findFirst as jest.Mock).mockRejectedValue(new Error('Database error'));
  ...
  describe('POST', () => {
    it('should return 401 for unauthorized requests', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/integrations/pinterest', {
        method: 'POST',
        body: JSON.stringify({ boardId: 'test', accessToken: 'test' }),
      });
    ...
    it('should create integration successfully', async () => {
      ...
      (PinterestService as jest.Mock).mockImplementation(() => mockService);

      const mockIntegration = {
        id: 'int-1',
        isActive: true,
      };
      (prisma.pinterestIntegration.upsert as jest.Mock).mockResolvedValue(mockIntegration);
