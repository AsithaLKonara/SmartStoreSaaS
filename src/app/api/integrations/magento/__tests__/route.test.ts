...
jest.mock('@/lib/integrations/magento/magentoService', () => ({
  MagentoService: jest.fn().mockImplementation(() => ({
    testConnection: jest.fn(),
    getOrCreateCatalog: jest.fn(),
  })),
}));

describe('/api/integrations/magento', () => {
  const mockSession = {
    user: {
      id: 'user-1',
  ...
  describe('GET', () => {
    it('should return 401 for unauthorized requests', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/integrations/magento');
    ...
    it('should return integration when found', async () => {
      ...
      (prisma.magentoIntegration.findFirst as jest.Mock).mockResolvedValue(mockIntegration);
    ...
    it('should return 404 when integration not found', async () => {
      (prisma.magentoIntegration.findFirst as jest.Mock).mockResolvedValue(null);
    ...
    it('should return 500 on server error', async () => {
      (prisma.magentoIntegration.findFirst as jest.Mock).mockRejectedValue(new Error('Database error'));
  ...
  describe('POST', () => {
    it('should return 401 for unauthorized requests', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/integrations/magento', {
        method: 'POST',
        body: JSON.stringify({ baseUrl: 'test', accessToken: 'test' }),
      });
    ...
    it('should create integration successfully', async () => {
      ...
      (MagentoService as jest.Mock).mockImplementation(() => mockService);

      const mockIntegration = {
        id: 'int-1',
        isActive: true,
      };
      (prisma.magentoIntegration.upsert as jest.Mock).mockResolvedValue(mockIntegration);
