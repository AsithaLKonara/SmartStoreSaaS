...
export function InstagramIntegration({ organizationId }: InstagramIntegrationProps) {
  const [integration, setIntegration] = useState<InstagramIntegrationData | null>(null);
  ...
  const loadIntegration = async () => {
    try {
      ...
      console.error('Error loading Instagram integration:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetup = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert('Instagram integration configured successfully!');
      } else {
        ...
        {integration?.id ? (
          <div className="space-y-4">
            <div className="text-sm text-gray-600">
              <p><strong>Business Account ID:</strong> {integration.businessAccountId}</p>
            ...
            >
              ...
              {syncLoading ? 'Syncing...' : 'Sync Products to Instagram'}
            </Button>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <Button
              onClick={handleSetup}
              disabled={setupLoading || !config.businessAccountId || !config.accessToken}
            >
              {setupLoading ? 'Setting up...' : 'Setup Instagram Integration'}
            </Button>
          </div>
        )}
      </CardContent>
